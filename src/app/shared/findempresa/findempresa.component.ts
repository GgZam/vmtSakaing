...
export class FindempresaComponent {
  ...
  constructor ( private dialogService: DialogService, private EmpresaService: EmpresaService ) {}
  ...
  findEmpresas() {
    ...
    this.EmpresaService.findCatalogoEmpresa(rqCatalogoEmpresa).subscribe({
      next: (resp) => {
        if(resp.codeResp === 'Ok'){
